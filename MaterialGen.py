...
elements = ["Si", "O", "C", "N", "Al", "Ga", "Cd", "Te", "Ge", "Se", "S", "As", "B", "Zn", "Cu", "P", "Pb", "Sn", "Mo", "In", "Ag"]
            ...
            for i in tqdm(els):
                for z in elements:
                    try:
                        ...
                        df.to_csv(f"./2_gen_mbj_bandgap/material_generated_samples.csv", index=False)
                    except:
                        pass
            ...
            for t in np.arange(1,3): 
                for p in elements:
                    for i in tqdm(elements):
                        for z in elements:
                            try:
                                comp = Composition.from_dict({i: m, p: t, z: n})
                                mbj_value = round(random.uniform(2.5, 5), 3)
                                prompt_example = "Below is a description of a superconductor material. Write a response that appropriately completes the request.\n\n### Instruction:\nGenerate atomic structure description with lattice lengths, angles, coordinates and atom types.\n\n### Input:\nThe chemical formula is " + comp.reduced_formula + ". The  mbj_bandgap value is "+ str(mbj_value)+ ".\n\n### Response:\n"
                                batch = llm_tokenizer(prompt_example, return_tensors="pt")
                                batch = {k: v.cuda() for k, v in batch.items()}
                                ...
                                try:
                                    ...
                                    print(index, e)
                                print(index, comp.reduced_formula, mbj_value)
                                df.to_csv(f"./2_gen_mbj_bandgap/material_generated_samples_ternary_onlyelementlist1.csv", index=False)
                            except:
                                pass
