...
for idx, name in enumerate(fourbit_models):
        ...
        data_test = "/home/jipengsun/LLM_Atom_Gen/2_gen_mbj_bandgap/llama-3-8b-bnb-4bit_generated_samples_updated.csv" #llama-3-8b-bnb-4bit_generated_samples_updated_temp
        df = pd.read_csv(data_test)
        #df = pd.DataFrame(data_test)

        #Look at each models
        for jdx, nname in df.iterrows():
                # if df.isna().loc[jdx, 'gen_material_str'] != True or jdx == 282 or jdx == 594:
                #     continue
                ...
                try:
                    ...
                    print("rms_dist.......")
                    print(atoms_pred, atoms_tar)
                    rms_dist = matcher.get_rms_anonymous(atoms_pred, atoms_tar) #594
                    ...
                    try:
                        print("out_data_tarpmg_to_atoms.....")           
